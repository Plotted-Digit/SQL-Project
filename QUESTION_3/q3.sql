...
with --Make a temp table from the existing tables
    TempTable as (
        ...
        FROM
            salesorderheader --Do required joins for retiving data from another tables
        ...
        group by
            SalesReasonID --Make a table with proper order of Sale amount
        order by
            Sales desc
...
from
    TempTable --Ordered the table data as per the Sale amount
order by
    Sales desc;